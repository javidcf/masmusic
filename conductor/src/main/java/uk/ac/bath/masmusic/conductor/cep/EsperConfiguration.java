...
public class EsperConfiguration {
    ...
    public com.espertech.esper.client.Configuration epConfiguration() {
        ...
        config.addPlugInSingleRowFunction("noteOnset",
                "uk.ac.bath.masmusic.conductor.cep.EsperConfiguration",
                "noteOnset");
        return config;
    }
    ...
    public static Onset noteOnset(TimeSpanNote note) {
        int absolutePitch = note.getPitch().getNote().getNumber()
                + (note.getPitch().getOctave() + 1) * 12;
        return new Onset(note.getTimestamp(), note.getDuration(), absolutePitch,
                note.getVelocity());
    }
}
